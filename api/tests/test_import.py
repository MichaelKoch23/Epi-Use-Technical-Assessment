...
async def test_duplicate_email_within_the_file_is_blocked_per_row(db_session):
    ...
    plan = await service.validate(
        [
            _row("E1", email="shared@example.com"),
            _row("E2", email="shared@example.com"),
        ]
    )

    assert plan.blocked_count == 2
    assert all("appears more than once" in row.reason for row in plan.rows)


async def test_email_already_held_by_another_employee_is_blocked(
    db_session, employee_factory
):
    await employee_factory(employee_number="E-EXISTING", email="taken@example.com")
    service = ImportService(db_session)

    plan = await service.validate([_row("E-NEW", email="taken@example.com")])

    assert plan.blocked_count == 1
    assert "already used by employee 'E-EXISTING'" in plan.rows[0].reason
...
async def test_a_row_keeping_its_own_email_is_not_a_collision(
    ...
    plan = await service.validate([_row("E-SAME", email="same@example.com")])

    assert plan.blocked_count == 0
    assert plan.rows[0].outcome == "will_update"
...
async def test_blank_manager_column_promotes_an_employee_to_root(
    ...
    service = ImportService(db_session)
    plan = await service.validate([_row("E-REPORT", manager="")])
    result = await service.commit(plan, actor_id=actor_id)
    await db_session.commit()

    assert result.committed
    refreshed = await EmployeeRepository(db_session).get(report.id)
...
async def test_manager_column_still_assigns_a_manager(
    ...
    service = ImportService(db_session)
    plan = await service.validate([_row("E-REPORT2", manager="E-BOSS2")])
    result = await service.commit(plan, actor_id=actor_id)
    await db_session.commit()
