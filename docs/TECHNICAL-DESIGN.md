...
graph TD
    ...
    DB[(Neon PostgreSQL 18<br/>aws-eu-central-1<br/>pooled endpoint)]
    GRV[Gravatar CDN]
...
One gap in that argument is worth naming: the suite pins `postgres:17-alpine` while the deployed database is PostgreSQL 18. Every construct the schema relies on - `WITH RECURSIVE`, deferred constraint triggers, GiST exclusion constraints, partial indexes, `NUMERIC` - is long-stable across both majors, so the two agree in practice. Pinning the container to the deployed major is nonetheless the correct fix, and is a deliberate to-do rather than an oversight.
...
|---|---|---|---|
| Database engine | PostgreSQL 18 | Recursive CTEs, deferred constraint triggers, partial indexes, exact `NUMERIC` for salary, and transactional integrity for the acyclicity invariant. Alternatives considered: MySQL, Firestore, MongoDB, Neo4j, Oracle | Schema changes require migrations |
...
| Managed database | Neon | A free tier that does not expire, PostgreSQL 18, and per-branch databases that make preview environments real rather than notional | No African region (section 10.2) |
